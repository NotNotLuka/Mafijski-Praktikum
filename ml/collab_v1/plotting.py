...
def plot_sig_bkg_from_np_arrays(features, labels, feature_names, logy=False):
    ...
    plt.figure(figsize=(len(features), 10))
    for i in range(num_features):
        ...
        plt.hist(
            bkg_list[i], density=True, bins=100, histtype="step", label="Bkg", color="b"
        )
...
def comp_plot(ptitle, vnames, reald, generd, inputd=None, logy=False):
    ...
    for i in range(feat_dim):
        ...
        # ax.set_xlim([-10.,10.])
        ax_range = (f_min[i], min(f_max[i], 5.0))
        # ax_range=(-6.,6.)
        # plt.xticks([])
        plt.yticks([])
        plt.grid(True)
