...
CONT_VARS = [0, 1, 3, 5, 6, 8, 9, 12, 17, 20, 21, 22, 24, 25, 26, 27]
CONT_VARS = [i for i in range(28)]

# BPK local data management in pickled files
datasets_root = "data/"
INPUT_HIGGS = "higgs_csv/"
...
class HIGGS:
    ...
    def __init__(
        self, train_start=None, train_count=None, test_start=None, test_count=None
    ):
        """
        arguments:
        ...
        self.train_count = 5_000_000 if train_count is None else train_count
        self.test_start = 10_000_000 if test_start is None else test_start
        self.test_count = 500_000 if test_count is None else test_count
    ...
    def process_data(self, path):
        ...
        if os.path.isfile(temp_filename):
            print(
                "data_dir already has the downloaded data file: {}".format(
                    temp_filename
                )
            )
        else:
        ...
        feature_names = [
            fname.replace(" ", "-")
            for fname in np.concatenate((["hlabel"], HIGGS_COL_NAMES))
        ]
        ...
        self.feature_names = pd.Series(
            np.concatenate((["hlabel"], sel_names))
        )  # add the label!
        z_sel_vars = [0] + sel_vars  # add the label!
        data_train = data.iloc[
            self.train_start : self.train_start + self.train_count, z_sel_vars
        ]
        data_val = data.iloc[
            self.test_start : self.test_start + self.test_count, z_sel_vars
        ]
